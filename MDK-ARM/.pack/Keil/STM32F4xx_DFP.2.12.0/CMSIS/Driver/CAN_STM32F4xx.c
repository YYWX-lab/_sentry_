...
#define ARM_CAN_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(1,13)        // CAN driver version
...
static int32_t CANx_MessageSend (uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, const uint8_t *data, uint8_t size, uint8_t x) {
  ...
  if (msg_info->rtr != 0U) {                            // If send RTR requested
    ...
    ptr_CAN->sTxMailBox[obj_idx].TDLR = __UNALIGNED_UINT32_READ(data   );
    ptr_CAN->sTxMailBox[obj_idx].TDHR = __UNALIGNED_UINT32_READ(data+4U);
