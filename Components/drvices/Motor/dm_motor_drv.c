...
* @param[in]:   obj:    指向CAN_HandleTypeDef结构的指针
...
************************************************************************
**/
void dm_motor_enable(CAN_Object_t* obj, motor_t *motor)
{
	switch(motor->ctrl.mode)
	{
		case mit_mode:
			enable_motor_mode(obj, motor->id, MIT_MODE);
			break;
		case pos_mode:
			enable_motor_mode(obj, motor->id, POS_MODE);
			break;
		case spd_mode:
			enable_motor_mode(obj, motor->id, SPD_MODE);
			break;
		case psi_mode:
			enable_motor_mode(obj, motor->id, PSI_MODE);
			break;
	}	
}
/**
...
************************************************************************
**/
void dm_motor_disable(CAN_Object_t* obj, motor_t *motor)
{
	switch(motor->ctrl.mode)
	{
		case mit_mode:
			disable_motor_mode(obj, motor->id, MIT_MODE);
			break;
		case pos_mode:
			disable_motor_mode(obj, motor->id, POS_MODE);
			break;
		case spd_mode:
			disable_motor_mode(obj, motor->id, SPD_MODE);
			break;
		case psi_mode:
			disable_motor_mode(obj, motor->id, PSI_MODE);
			break;
	}	
...
************************************************************************
**/
void dm_motor_ctrl_send(CAN_Object_t* obj, motor_t *motor)
{
	switch(motor->ctrl.mode)
	{
		case mit_mode:
			mit_ctrl(obj, motor, motor->id, motor->ctrl.pos_set, motor->ctrl.vel_set, motor->ctrl.kp_set, motor->ctrl.kd_set, motor->ctrl.tor_set);
			break;
		case pos_mode:
			pos_ctrl(obj, motor->id, motor->ctrl.pos_set, motor->ctrl.vel_set);
			break;
		case spd_mode:
			spd_ctrl(obj, motor->id, motor->ctrl.vel_set);
			break;
		case psi_mode:
			psi_ctrl(obj, motor->id,motor->ctrl.pos_set, motor->ctrl.vel_set, motor->ctrl.cur_set);
			break;
	}	
}
...
* @param[in]:   obj: 	 指向CAN控制结构体的指针
* @param[in]:  	motor:   指向电机结构体的指针
* @retval:     	void
* @details:    	根据电机的控制模式，调用对应模式的清除错误函数
************************************************************************
**/
void dm_motor_clear_err(CAN_Object_t* obj, motor_t *motor)
{
	switch(motor->ctrl.mode)
	{
		case mit_mode:
			clear_err(obj, motor->id, MIT_MODE);
			break;
		case pos_mode:
			clear_err(obj, motor->id, POS_MODE);
			break;
		case spd_mode:
			clear_err(obj, motor->id, SPD_MODE);
			break;
		case psi_mode:
			clear_err(obj, motor->id, PSI_MODE);
			break;
	}	
}
/**
...
void dm_motor_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	motor->para.pos = uint_to_float(motor->para.p_int, -motor->tmp.PMAX, motor->tmp.PMAX, 16); // (-3.14159,3.14159)


	motor->para.vel = uint_to_float(motor->para.v_int, -motor->tmp.VMAX, motor->tmp.VMAX, 12); // (-30.0,30.0)
	motor->para.tor = uint_to_float(motor->para.t_int, -motor->tmp.TMAX, motor->tmp.TMAX, 12); // (-10.0,10.0)
...
************************************************************************
**/
void enable_motor_mode(CAN_Object_t* obj, uint16_t motor_id, uint16_t mode_id)
{
	uint8_t data[8];
	...
	data[6] = 0xFF;
	data[7] = 0xFC;
	BSP_CAN_TransmitData(obj, id, data, 8);
	// canx_send_data(obj, id, data, 8);
}
/**
************************************************************************
...
************************************************************************
**/
void disable_motor_mode(CAN_Object_t* obj, uint16_t motor_id, uint16_t mode_id)
{
	uint8_t data[8];
	uint16_t id = motor_id + mode_id;
...
************************************************************************
**/
void save_pos_zero(CAN_Object_t* obj, uint16_t motor_id, uint16_t mode_id)
{
	uint8_t data[8];
	uint16_t id = motor_id + mode_id;
...
************************************************************************
**/
void clear_err(CAN_Object_t* obj, uint16_t motor_id, uint16_t mode_id)
{
	uint8_t data[8];
	uint16_t id = motor_id + mode_id;
...
* @param[in]:   obj:			指向CAN_HandleTypeDef结构的指针，用于指定CAN总线
* @param[in]:   motor_id:	电机ID，指定目标电机
* @param[in]:   pos:			位置给定值
* @param[in]:   vel:			速度给定值
* @param[in]:   kp:				位置比例系数
...
************************************************************************
**/
void mit_ctrl(CAN_Object_t* obj, motor_t *motor, uint16_t motor_id, float pos, float vel,float kp, float kd, float tor)
{
	uint8_t data[8];
...
************************************************************************
**/
void pos_ctrl(CAN_Object_t* obj,uint16_t motor_id, float pos, float vel)
{
	uint16_t id;
	uint8_t *pbuf, *vbuf;
	uint8_t data[8];
...
************************************************************************
**/
void spd_ctrl(CAN_Object_t* obj, uint16_t motor_id, float vel)
{
	uint16_t id;
	uint8_t *vbuf;
	uint8_t data[4];
	...
	// canx_send_data(obj, id, data, 4);
	BSP_CAN_TransmitData(obj, id, data, 4);
}
...
************************************************************************
**/
void psi_ctrl(CAN_Object_t* obj, uint16_t motor_id, float pos, float vel, float cur)
{
	uint16_t id;
...
void read_motor_data(uint16_t id, uint8_t rid) 
{
	...
	// canx_send_data(&obj1, 0x7FF, data, 4);
	// BSP_CAN_TransmitData(obj, 0x7FF, data, 4);
}
...
void read_motor_ctrl_fbdata(uint16_t id) 
{
	...
	// canx_send_data(&obj1, 0x7FF, data, 4);
	// BSP_CAN_TransmitData(obj, 0x7FF, data, 8);
}
/**
************************************************************************
...
void write_motor_data(uint16_t id, uint8_t rid, uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3)
{
	...
	// canx_send_data(&obj1, 0x7FF, data, 8);
	// BSP_CAN_TransmitData(obj, 0X7FF, data, 8);
}
/**
************************************************************************
